...
namespace GameName3.Render.DrawingEffects
{
	...
	public class ShadowmapResolver
	{
		...
		public ShadowmapResolver(GraphicsDevice graphicsDevice, QuadRenderComponent quadRender, int maxShadowmapSize, int maxDepthBufferSize)
		{
			this.graphicsDevice = graphicsDevice;
			this.quadRender = quadRender;

			reductionChainCount = (int)(Math.Log(maxShadowmapSize, 2));
			baseSize = 2 << reductionChainCount;
			depthBufferSize = maxDepthBufferSize;
		}

		public void LoadContent(ContentManager content)
		{
			reductionEffect = content.Load<Effect>("Graphical Effects\\Effect Files\\ShadowReduction");
			resolveShadowsEffect = content.Load<Effect>("Graphical Effects\\Effect Files\\Shadow");
