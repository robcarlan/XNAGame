...
namespace WindowsGame1.Entity_Manager
{
    class InventoryManager
    {
        ...
        public InventoryManager(DataLoader.ItemLoader[] _items, DataLoader.ConsumableItemLoader[] _consumableItems,
                        DataLoader.ArmourItemLoader[] _armour, DataLoader.WeaponItemLoader[] _weapons, ref Dictionary<short, Point> spriteLocations)
        {
            errorMessages = new List<string>();
            items = new ItemManager(_items, _consumableItems, _armour, _weapons, out errorMessages);
        ...
        public short getItemQuantity(short itemID, short objectID)
        {
            ...
            for (int i = 0; i < inventories[objectID].contents.Capacity; i++)
            {
                if (inventories[objectID].contents[i] == itemID)
                {
                    ...
                    if (inventories[objectID].quantity[i] == _quantity)
                    {
						...
						inventories[objectID].contents[i] = inventories[objectID].quantity[i] = 0;
