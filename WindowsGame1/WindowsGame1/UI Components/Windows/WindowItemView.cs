...
namespace WindowsGame1.UI_Components.Windows
{
    public class WindowItemView : WindowRegion
    {
        ...
        public WindowItemView(int _x, int _y, int _width, int _height, string _namePrefix, int _spriteX, int _spriteY, float _scale, 
            ...
            : base(_x, _y, _width, _height, _namePrefix, _spriteX, _spriteY, _scale)
        {
            ...
            for (short ItemNumber = 0; ItemNumber < maxVisibleItems; ItemNumber++)
            {
                shownItems.Add( new WindowItemRegion(
                    ...
                    itemWidth - 16 - (int)(ScrollBarScale.X * spriteDimensions.X), itemHeight, "sel", 16, 16, 2f, items[ItemNumber].Name, items[ItemNumber].description[0], 0)
                    );
