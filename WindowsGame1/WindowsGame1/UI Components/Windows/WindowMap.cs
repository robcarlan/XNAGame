...
namespace WindowsGame1.UI_Components.Windows
{
	public class WindowMap : WindowBase 
	{
        ...
        public WindowMap(Texture2D _Texture, Vector2 _origin, Vector2 _size, SpriteList _windowSprites,
            ...
            : base(_Texture, _origin, new Vector2(WINDOW_WIDTH, WINDOW_HEIGHT), _windowSprites, _mousePos, _screen, _font, "World Map" )
        {
            ...
            //Get the map Data
            mapDimensions.X = windowSprites.list["wMap"][0].X;
            mapDimensions.Y = windowSprites.list["wMap"][0].Y;
            ...
            //Set regions
            mapRegion = new WindowRegion(mapWindowLocation.X, mapWindowLocation.Y, MAP_WINDOW_WIDTH, MAP_WINDOW_HEIGHT, "map",
                 _windowSprites.list["mapTLCnr"][0].Width, _windowSprites.list["mapTLCnr"][0].Height, 2f); 

            //Set text Position
            mapTextOrigin.X = mapWindowLocation.X + (MAP_WINDOW_WIDTH - _windowSprites.list["mapText"][0].Width * mapTextScale) / 2;
            mapTextOrigin.Y = mapWindowLocation.Y - (_windowSprites.list["mapText"][0].Height * mapTextScale);
        }
        ...
        public void moveMap( Point mouseChange )
        {
            ...
            mapDimensions.X = (int)MathHelper.Clamp(mapDimensions.X - mouseChange.X, windowSprites.list["wMap"][0].X, windowSprites.list["wMap"][0].Width + windowSprites.list["wMap"][0].X - mapWindowLocation.Width);
            mapDimensions.Y = (int)MathHelper.Clamp(mapDimensions.Y - mouseChange.Y, windowSprites.list["wMap"][0].Y, windowSprites.list["wMap"][0].Height + windowSprites.list["wMap"][0].Y - mapWindowLocation.Height);
        }
        ...
        public override void draw(ref SpriteBatch sprite)
        {
            ...
            //Draw Text
            sprite.Draw(windowTex, mapTextOrigin, windowSprites.list["mapText"][0], Color.White, 0f, Vector2.Zero, mapTextScale, SpriteEffects.None, 0f);
        }
